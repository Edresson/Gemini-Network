...
from models.gemini import GeminiV1
from utils.audio_processor import AudioProcessor 

def validation(criterion, model, c, testloader, tensorboard, step,  cuda):
    model.zero_grad()
    model.eval()
    loss = 0 
    acc = 0
...
def train(args, log_dir, checkpoint_path, trainloader, testloader, tensorboard, c, model_name, cuda=True):
    if(model_name == 'geminiv1'):
        model = GeminiV1(c):
    else:
